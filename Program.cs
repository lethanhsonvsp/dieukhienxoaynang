...
namespace dieukhienxoaynang
{
    class Program
    {
        ...
        static void StartCanOpen()
        {
            ...
            {
                Console.WriteLine("Không lấy được ifindex của can0!");
                return;
            }

            // 3. Bind socket tới can0
            ...
            {
                Console.WriteLine("Không bind được socket tới can0!");
                return;
            }

            Console.WriteLine($"SocketCAN opened & bound to can0 (fd={sock})");
        }

        // static void EnableMotorVelocityMode()
        // {
        //     uint sdoTxId = 0x600 + (uint)nodeId;
        //     ControlPDO.SendFrame(sock, sdoTxId, new byte[] { 0x2F, 0x60, 0x60, 0x00, 0x03, 0x00, 0x00, 0x00 });
        //     Thread.Sleep(50);
        //     ControlPDO.SendFrame(sock, sdoTxId, new byte[] {0x40, 0x61, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00 });
        //     Thread.Sleep(100);
        //     ControlPDO.SendFrame(sock, sdoTxId, new byte[] { 0x2B, 0x40, 0x60, 0x00, 0x06, 0x00, 0x00, 0x00 });
        //     Thread.Sleep(50);
        //     ControlPDO.SendFrame(sock, sdoTxId, new byte[] { 0x2B, 0x40, 0x60, 0x00, 0x07, 0x00, 0x00, 0x00 });
        //     Thread.Sleep(50);
        //     ControlPDO.SendFrame(sock, sdoTxId, new byte[] { 0x2B, 0x40, 0x60, 0x00, 0x0F, 0x00, 0x00, 0x00 });
        //     ControlPDO.SendFrame(sock, sdoTxId, new byte[] { 0x23, 0xFF, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00});
        // }
